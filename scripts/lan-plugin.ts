...
export async function startLanWsServer(): Promise<void> {
      ...
      console.log(`[峡谷身份战] 局域网主机 ws://${lanAddress()}:8788`);

      const pushLobby = (room: InstanceType<typeof GameRoom>) => {
        const payload = JSON.stringify({
          type: "lobby",
          seats: room.clients.map((client) => ({ seat: client.seat, name: client.name })),
        });
        ...
        socket.on("message", (raw) => {
          ...
          try {
            if (message.type === "join") {
              ...
              send({ type: "hello", playerId: seat, room: roomId, mode: room.mode });
              pushLobby(room);
              return;
            }
